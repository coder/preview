...
locals {
  ...
  validation = {
    regex = "^(?:[A-Za-z]{5})?$"
    error = "You must enter a valid 5 letter english word."
  }
  ...
  remove_letters =  setunion(
    coalesce(toset(module.check_one.unmatching), []),
    coalesce(toset(module.check_two.unmatching), []),
    coalesce(toset(module.check_three.unmatching), []),
    coalesce(toset(module.check_four.unmatching), []),
    coalesce(toset(module.check_five.unmatching), []),
    coalesce(toset(module.check_six.unmatching), []),
  )
  ...
  solved = (data.coder_parameter.one.value == local.correct ||
...
output "debug" {
  value = local.remaining
}

data "coder_parameter" "letter_bank" {
  name = "letter_bank"
  display_name = local.solved ? "Congrats! You got the word!" : "Letter bank"
  description = local.solved ? "Enter your workspace name" : "Remaining available letters: ${join("", local.remaining)}"
  type = "string"
  order = 9
  default = ""
  form_type = "input"
...
data "coder_parameter" "one" {
  ...
  validation {
    ...
    invalid = length(data.coder_parameter.one.value) == 5 && !contains(local.wordlist, data.coder_parameter.one.value)
  }
}
...
data "coder_parameter" "two" {
  ...
  validation {
    regex = local.validation.regex
    error = local.validation.error
    invalid = length(data.coder_parameter.two.value) == 5 && !contains(local.wordlist, data.coder_parameter.two.value)
  }
}
...
module "check_two" {
  source = "./checker"
  correct = local.correct
  previous = data.coder_parameter.two.value
}
...
data "coder_parameter" "three" {
  ...
  validation {
    regex = local.validation.regex
    error = local.validation.error
    invalid = length(data.coder_parameter.three.value) == 5 && !contains(local.wordlist, data.coder_parameter.three.value)
  }
}
...
module "check_three" {
  source = "./checker"
  correct = local.correct
  previous = data.coder_parameter.three.value
}
...
data "coder_parameter" "four" {
  ...
  validation {
    regex = local.validation.regex
    error = local.validation.error
    invalid = length(data.coder_parameter.four.value) == 5 && !contains(local.wordlist, data.coder_parameter.four.value)
  }
}
...
module "check_four" {
  source = "./checker"
  correct = local.correct
  previous = data.coder_parameter.four.value
}
...
data "coder_parameter" "five" {
  ...
  validation {
    regex = local.validation.regex
    error = local.validation.error
    invalid = length(data.coder_parameter.five.value) == 5 && !contains(local.wordlist, data.coder_parameter.five.value)
  }
}
...
module "check_five" {
  source = "./checker"
  correct = local.correct
  previous = data.coder_parameter.five.value
}
...
data "coder_parameter" "six" {
  ...
  validation {
    regex = local.validation.regex
    error = local.validation.error
    invalid = length(data.coder_parameter.six.value) == 5 && !contains(local.wordlist, data.coder_parameter.six.value)
  }
}
...
module "check_six" {
  source = "./checker"
  correct = local.correct
  previous = data.coder_parameter.six.value
}
